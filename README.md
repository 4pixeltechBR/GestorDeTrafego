<div align="center">
  ...
  **O seu Copiloto de Tráfego Pago Open Source e Custo Zero.**
  ...
  <p align="center">
    Uma arquitetura multi-agente <strong>Omnichannel (Meta + Google)</strong> que interpreta briefings casuais, cria copys persuasivas ou anúncios de pesquisa (RSA), valida contra as políticas, e sobe campanhas automaticamente—tudo gerenciado diretamente pelo <strong>Telegram</strong> no seu celular ou desktop.
  </p>
</div>
...
1. **Privacidade Absoluta:** O banco de dados (SQLite), seus Tokens de API e seu orçamento ficam trancados na **sua máquina**. Nenhuma credencial sua vai para a nuvem.
...
3. **Simplicidade:** Dois cliques no Windows (`start.bat`) e seu Bot no Telegram já acorda esperando seu briefing. Sem complicação, sem senhas, sem painéis pesados.
...
O usuário final recebe a resposta pronta no Telegram, sem interrupções e sem pagar $1 dólar à OpenAI ou Anthropic.
...
1. **Groq (`llama-3.3-70b-versatile`)**:
   ...
   - **Onde o sistema usa:** *Orquestrador* (raciocínio rápido), *Keyword Planner* e *Executor* (geração pura de JSON com zero margem de formatação errada).
2. **Google AI Studio (`gemini-2.0-flash`)**:
   ...
   - **Onde o sistema usa:** *Copywriter* (criatividade persuasiva) e *Analista* (digestão de números grandes e métricas).
...
- 🔍 **Keyword Planner:** O farejador do Google Search. Extrai palavras-chave exatas, de frase e amplas, além de blindar a campanha listando palavras negativas (Ex: "grátis", "como fazer").
...
- 📐 **Executor:** Envelopa os dados validados e bifurca o sistema construindo o pacote JSON Graph API ou os frames gRPC mutáveis do Google Ads. (Sempre enviando as opções para você aprovar no Telegram).
...
- Um Bot no Telegram criado através do [BotFather](https://t.me/botfather).
...
```
Abra o ".env" no bloco de notas e insira suas credenciais:
- Chaves de IA: [Groq Console](https://console.groq.com/keys) e [Google AI Studio](https://aistudio.google.com/app/apikey)
- Telegram: Seu `TELEGRAM_BOT_TOKEN` e o seu ID de administrador `TELEGRAM_ADMIN_CHAT_ID`.
- APIs de Anúncios: IDs e Tokens da sua conta do Meta Ads e/ou Google Ads.
...
▶️ start.bat
```
Ele vai baixar todas as bibliotecas necessárias, preparar o banco de dados e conectar seu bot. Basta abrir o Telegram e mandar sua primeira mensagem com um briefing de campanha!
...
O projeto inclui **Guardrails Rígidos em Python e SQLite**. 
Não importa o que a IA faça: se no `.env` está definido `BUDGET_MAX_DIARIO=100`, qualquer solicitação de criação de campanha que vá exceder R$100,00 da conta diária é barrada localmente e notificada a você. A IA nunca tocará na porta da Meta API nestes casos.
